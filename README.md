...
> ✅ **TRẠNG THÁI CODE:**
> Toàn bộ khối bài tập trong `src/` đã được hoàn thiện và không còn placeholder chưa triển khai. Luồng nghiệm thu và lệnh tái hiện được mô tả trong [Guide.md](docs/Guide.md) và phần checklist bên dưới.
...
cp .env.example .env
```
Mở file `.env` và điền API Key tương ứng (hỗ trợ `gemini`, `openai`, `anthropic`, `openrouter`, `ollama`, `custom`, `mock`). Ví dụ cấu hình OpenRouter:
```dotenv
...
Không commit file `.env`. Chạy smoke test API/RAG sau khi điền key:

```powershell
python -c "from core.config import load_settings; from retrieval.index import LocalEmbeddingIndex; from retrieval.agent import build_agent, run_agent_question; s=load_settings(); idx=LocalEmbeddingIndex.load(s); print(run_agent_question(build_agent(s, idx), 'What are the main research topics in the indexed papers?'))"
```

### Kiểm thử tự động và coverage

```powershell
python -m pip install -e ".[dev]"
python -m pytest -p no:cacheprovider --cov=src --cov-report=term-missing
```

GitHub Actions chạy cùng test suite trên Python 3.11 và 3.13. Coverage gate bắt buộc đạt tối thiểu 80%.
...
- [ ] **Pha 2 (Corruption & Repair):** Lệnh `python script/run_corruption_flow.py` chạy thành công, tạo ra:
  ...
  - `data/results/repair_log.json` (bằng chứng tự động kích hoạt repair)
